    ...
    <div className = "container " onClick={()=>{setDropdownHamburger(!dropdownforHamburger)}}>
        ...
        <nav className="navBar">
            <div className="siteLogoSection">
                <Link to="/Home" relative="path"><img className="logoPng" id="hidden" src={LogoFullTextTicketLarge}/></Link>
            </div>
            ...
            <div className="optionsSection" >
                ...
                {navItemsLanding.map(item=> {
                        if (item.title === "Organize" ){
                            return (
                                <li key={item.id} id={item.id} className={item.cName} onMouseEnter={()=>{setDropdownOrganize(true)}} onMouseLeave={()=>{setDropdownOrganize(false)}}>
                                    <div className="organizeDiv">
                                        <Link to={item.path} >
                                            {item.title}
                                        </Link>

                                        <svg className="dropdownArrow" id="chevron-down-chunky_svg__eds-icon--chevron-down-chunky_svg" x="0" y="0" viewBox="0 0 24 24" xml:space="preserve"><path id="chevron-down-chunky_svg__eds-icon--chevron-down-chunky_base" fill-rule="evenodd" clip-rule="evenodd" d="M7 10.2l5 5 5-5-1.4-1.4-3.6 3.6-3.6-3.6z"></path></svg>
                                    </div>
                        ...
                        if (item.title === "Help" ){
                            return (    
                                <li key={item.id} id={item.id} className={item.cName} onMouseEnter={()=>{setDropdownHelp(true)}} onMouseLeave={()=>{setDropdownHelp(false)}}>
